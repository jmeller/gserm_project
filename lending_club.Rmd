...
data.wNAs.num <- data.num.features[, c("id", names(cols.wNAs)), with = F]
...
data.wNAs.num <- data.wNAs.num[,paste0("dmi_", impute.cols) := # add dmis
                         ...
                         .SDcols = impute.cols]
data.imputed <- data.wNAs.num[,(impute.cols) := 
...
train.data.subset <- train.data[sample(1:nrow(train.data), 5000), c("default", final.cols), with = F] # larger training sets kill my memory...
